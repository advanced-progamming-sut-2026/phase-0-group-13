...
public class QuestMenuController implements BaseController {
  ...
  public void handleinput(String command) {
    ...
    if (QuestMenuCommands.ShowQuests.getMatcher(command) != null) {
      ...
      int level;
      try {
        level = Integer.parseInt(matcher.group("level").trim());
      } catch (Exception e) {
        System.out.println("error: invalid level number.");
        return;
      }
      handlePlayMiniGame(name, level);
  ...
  private void handlePlayMiniGame(String name, int level) {
    ...
    if (level < 1 || level > 3) {
      System.out.println("error: minigames only have levels 1, 2, and 3.");
      return;
    }

    if (!progress.isMiniGameUnlocked(key)) {
      System.out.println("error: you haven't unlocked this minigame yet.");
      return;
    }

...
public class AuthService {
  ...
  public static Result checkUsername(String username) {
    if (username == null || username.isEmpty()) {
      return new Result(false, "error: username cannot be empty", null);
    }
    if (!USERNAME_PATTERN.matcher(username).matches()) {
      return new Result(
              false,
              "error: username may only contain letters, digits, and the '-' symbol",
              null);
    }
    return new Result(true, "Username is valid", null);
  }

  public static Result checkNickname(String nickname) {
    if (nickname == null) {
      return new Result(false, "error: nickname cannot be empty", null);
    }
    int len = nickname.length();
    if (len < 3 || len > 30) {
      return new Result(
              false, "error: nickname length must be between 3 and 30 characters", null);
    }
    return new Result(true, "Nickname is valid", null);
  }

  public static Result checkGender(String gender) {
    if (gender == null) {
      return new Result(false, "error: gender cannot be empty", null);
    }
    String normalized = gender.trim().toLowerCase();
    if (!normalized.equals("male") && !normalized.equals("female")) {
      return new Result(false, "error: gender must be either 'male' or 'female'", null);
    }
    return new Result(true, "Gender is valid", null);
  }

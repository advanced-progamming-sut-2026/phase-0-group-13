...
public class AuthService {
    ...
    public static Result checkPassword(String password) {
        ...
        if (!(hasUpper && hasLower && hasDigit && hasSpecial)) {
            return new Result(false, "error: weak password. must contain uppercase, lowercase, numbers, and special characters.", null);
        }
    ...
    public static Result checkEmail(String email) {
        ...
        if (username.contains("..")) {
            return new Result(false, "error: dot (.) cannot appear twice consecutively in email username", null);
        }
        if (!Pattern.matches("^[a-zA-Z0-9]([a-zA-Z0-9_\\-\\.]*[a-zA-Z0-9])?$", username)) {
            return new Result(false, "error: email username must start/end with a letter/number and contain only valid characters", null);
        }
        ...
        if (!Pattern.matches("^[a-zA-Z0-9]([a-zA-Z0-9\\-]*[a-zA-Z0-9])?(\\.[a-zA-Z0-9]([a-zA-Z0-9\\-]*[a-zA-Z0-9])?)*$", domain)) {
            return new Result(false, "error: email domain contains invalid characters or starts/ends with invalid characters", null);
        }
